...
build_variant() {
  ...
  [[ -d templates ]] && { 
    ...
    find templates -type f -not -path "templates/commands/*" -not -name "vscode-settings.json" | while read -r f; do
...
ALL_AGENTS=(claude gemini copilot cursor-agent qwen opencode windsurf codex kilocode auggie roo codebuddy amp shai q agy bob qodercli cline generic)
ALL_SCRIPTS=(sh ps)
